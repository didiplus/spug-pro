...
import {Form, Tabs, DatePicker, InputNumber, Input, Button} from 'antd';
...
export default observer(function () {
  ...
  function handleSubmit() {
    if (trigger === 'date' && args['date'] <= dayjs()) {
      return message.error('任务执行时间不能早于当前时间')
    }
    setLoading(true)
    const formData = lds.pick(store.record, ['id', 'name', 'type', 'interpreter', 'command', 'desc', 'rst_notify', 'playbook_id']);
  ...
  function _parse_args() {
    switch (trigger) {
      case 'date':
        return dayjs(args['date']).format('YYYY-MM-DD HH:mm:ss');
      case 'cron':
        const {rule, start, stop} = args['cron'];
        return JSON.stringify({
          rule,
          start: start ? dayjs(start).format('YYYY-MM-DD HH:mm:ss') : null,
          stop: stop ? dayjs(stop).format('YYYY-MM-DD HH:mm:ss') : null
        });
      default:
        return args[trigger];
    }
    ...
    <Form layout="vertical" wrapperCol={{span: 14, offset: 6}}>
      <Form.Item>
        <Tabs activeKey={trigger} onChange={setTrigger} tabPosition="left" style={{minHeight: 200}} items={[
          ...
          {key: 'date', label: '一次性', children: (
            <Form.Item required label="执行时间" extra="仅在指定时间运行一次。">
              <DatePicker
                showTime
                disabledDate={v => v && v.format('YYYY-MM-DD') < dayjs().format('YYYY-MM-DD')}
                style={{width: 200}}
                placeholder="请选择执行时间"
                onOk={() => false}
                value={args['date'] ? dayjs(args['date']) : undefined}
                onChange={v => handleArgs('date', v)}/>
            </Form.Item>
          )},
          {key: 'cron', label: 'UNIX Cron', children: (
            <div>
              ...
              <Form.Item label="生效时间" extra="定义的执行规则在到达该时间后生效">
                <DatePicker
                  ...
                  style={{width: '100%'}}
                  placeholder="可选输入"
                  value={lds.get(args, 'cron.start') ? dayjs(args['cron']['start']) : undefined}
              ...
              <Form.Item label="结束时间" extra="执行规则在到达该时间后不再执行">
                <DatePicker
                  ...
                  style={{width: '100%'}}
                  placeholder="可选输入"
                  value={lds.get(args, 'cron.stop') ? dayjs(args['cron']['stop']) : undefined}
