...
import {Form, Select, Radio, Transfer, Checkbox, Button} from 'antd';
import { http } from 'libs';
import groupStore from '../alarm/group/store';
import policyStore from '../alarm/policy/store';
...
export default observer(function () {
  ...
  useEffect(() => {
    if (policyStore.records.length === 0) {
      policyStore.fetchRecords();
    }
  }, [])
  ...
  function handleSubmit() {
    ...
    if (formData['alarm_policy'] === undefined) {
      formData['alarm_policy'] = store.record.alarm_policy || null;
    }
  ...
  return (
    <Form form={form} labelCol={{span: 6}} wrapperCol={{span: 14}}>
      ...
      <Form.Item name="alarm_policy" initialValue={info.alarm_policy} label="告警策略" extra={<>去 <Link to="/alarm/policy">告警策略</Link> 管理升级/收敛规则。</>}>
        <Select placeholder="不使用策略" allowClear>
          {policyStore.records.filter(item => item.is_active).map(item => (
