...
import {Modal, Form, Input, Tooltip, Checkbox, Divider, theme, Typography} from 'antd';
...
export default observer(function () {
  ...
  function Test(props) {
    return testLoading === props.mode ? (
      <LoadingOutlined style={{fontSize: 16, color: token.colorPrimary}}/>
    ) : (
      <Tooltip title="执行测试">
        <ThunderboltOutlined
          style={{fontSize: 16, color: token.colorPrimary, cursor: 'pointer'}}
  ...
  return (
    <Modal
      ...
      <Form form={form} initialValues={{
        ...
        <Divider orientation="left" plain style={{margin: '8px 0 16px', borderColor: token.colorBorderSecondary, color: token.colorTextSecondary, fontSize: token.fontSizeSM}}>通知渠道</Divider>
        {channelConfig.map(channel => (
          <div key={channel.key} style={{
            marginBottom: channels[channel.key] ? 16 : 4,
            padding: channels[channel.key] ? `${token.paddingXS}px ${token.paddingSM}px` : 0,
            background: channels[channel.key] ? token.colorFillQuaternary : 'transparent',
            ...
            {channels[channel.key] && channel.fields.map(field => {
              const extra = field.extra || (channel.help && field === channel.fields[0] ? (
                <Typography.Link type="secondary" style={{fontSize: token.fontSizeSM}}>
                  {channel.help.text}
