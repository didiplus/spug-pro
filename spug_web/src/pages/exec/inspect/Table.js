  ...
  render() {
    return (
      <TableCard
        ...
        <Table.Column title="巡检模板" dataIndex="template_name"/>
        <Table.Column title="目标主机数" dataIndex="host_ids" render={v => v ? v.length : 0}/>
        ...
        <Table.Column title="通知方式" dataIndex="notify_mode" width={120} render={v => (
          v && v.length > 0 ? v.map(m => <Tag key={m}>{modeMap[m] || m}</Tag>) : '-'
        )}/>
