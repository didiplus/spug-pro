...
import { Menu, Input, Button, Modal, Space, Radio, Form, Alert } from 'antd';
import {
  ArrowLeftOutlined,
  DiffOutlined,
  HistoryOutlined,
        ...
        <div className={styles.container}>
          <div className={styles.left}>
            <div style={{display: 'flex', alignItems: 'center', padding: '0 0 10px 10px'}}>
              <ArrowLeftOutlined onClick={() => history.goBack()} style={{fontSize: 16, marginRight: 8, cursor: 'pointer'}}/>
              <span style={{fontSize: 16, fontWeight: 500, flex: 1}}>环境列表</span>
              <Button type="link" icon={<DiffOutlined/>} onClick={store.showDiff}>对比配置</Button>
            </div>
            <Menu
              mode="inline"
              ...
              items={envStore.records.map(item => ({
                key: item.id, label: `${item.name} (${item.key})`, env: item
              }))}
            />
          </div>
