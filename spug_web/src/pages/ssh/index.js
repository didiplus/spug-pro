...
function WebSSH(props) {
  ...
  function TabRender(props) {
    ...
    return (
      <Dropdown trigger={['contextMenu']} menu={{onClick: ({key, domEvent}) => handeTabAction(key, host, domEvent), items: [
        {key: 'copy', icon: <CopyOutlined/>, label: '复制窗口'},
        {key: 'reconnect', icon: <ReloadOutlined/>, label: '重新连接'},
        {key: 'rClose', icon: <VerticalAlignBottomOutlined style={{transform: 'rotate(90deg)'}}/>, label: '关闭右侧'},
        {key: 'oClose', icon: <VerticalAlignMiddleOutlined style={{transform: 'rotate(90deg)'}}/>, label: '关闭其他'},
