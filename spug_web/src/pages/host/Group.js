...
export default observer(function () {
  ...
  const menuItems = [
    {key: '0', icon: <FolderOutlined/>, label: '新建根分组', onClick: handleAddRoot},
    {key: '1', icon: <FolderAddOutlined/>, label: '新建子分组', onClick: handleAdd},
    {key: '2', icon: <EditOutlined/>, label: '重命名', onClick: () => setAction('edit')},
    {type: 'divider'},
    {key: '3', icon: <CopyOutlined/>, label: '添加主机', onClick: () => store.showSelector(true)},
    {key: '4', icon: <ScissorOutlined/>, label: '移动主机', onClick: () => store.showSelector(false)},
    {key: '5', icon: <CloseOutlined/>, danger: true, label: '删除主机', onClick: handleRemoveHosts},
    {type: 'divider'},
    {key: '6', icon: <DeleteOutlined/>, danger: true, label: '删除此分组', onClick: handleRemove},
  ]
  ...
  return (
    <Card
      ...
      <Spin spinning={store.grpFetching}>
        <Dropdown
          menu={{items: menuItems, onClick: () => setVisible(false)}}
          open={visible}
          trigger={['contextMenu']}
