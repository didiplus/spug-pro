...
import {Drawer, Descriptions, List, Button, Input, Select, DatePicker, Tag} from 'antd';
...
export default observer(function () {
  ...
  function handleSubmit() {
    setLoading(true)
    if (host.created_time) host.created_time = dayjs(host.created_time).format('YYYY-MM-DD')
    if (host.expired_time) host.expired_time = dayjs(host.expired_time).format('YYYY-MM-DD')
      ...
      <Descriptions
        ...
        <Descriptions.Item label="内网IP">
          {lds.get(host, 'private_ip_address', []).map((item, index) => (
            <Tag closable={edit} key={index} onClose={() => handleTagRemove('sip', index)}>{item}</Tag>
          ))}
        ...
        <Descriptions.Item label="公网IP">
          {lds.get(host, 'public_ip_address', []).map((item, index) => (
            <Tag closable={edit} key={index} onClose={() => handleTagRemove('gip', index)}>{item}</Tag>
          ))}
