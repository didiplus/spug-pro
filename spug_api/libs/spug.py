...
from apps.alarm.models import Group, Contact, ContactChannel
...
class Notification:
    ...
    def dispatch_monitor(self, modes):
        ...
        for mode in modes:
            if mode == '1':
                targets.update(x for x in push_ids if x.startswith('wx_mp_'))
            elif mode == '2':
                targets.update(x for x in push_ids if x.startswith('sms_'))
            elif mode == '3':
                channels = ContactChannel.objects.filter(contact_id__in=u_ids, type='ding')
                users = [(c.identifier, c.secret) for c in channels]
                if not users:
                    Notify.make_monitor_notify(
                        '发送报警信息失败',
                ...
                channels = ContactChannel.objects.filter(contact_id__in=u_ids, type='email')
                users = set(c.identifier for c in channels)
                if not users:
                    if not mail_ids:
                        Notify.make_monitor_notify(
                            '发送报警信息失败',
                ...
                channels = ContactChannel.objects.filter(contact_id__in=u_ids, type='qy_wx')
                users = set(c.identifier for c in channels)
                if not users:
                    Notify.make_monitor_notify(
                        '发送报警信息失败',
                ...
                targets.update(x for x in push_ids if x.startswith('voice_'))
            elif mode == '7':
                channels = ContactChannel.objects.filter(contact_id__in=u_ids, type='feishu')
                users = [(c.identifier, c.secret) for c in channels]
                if not users:
                    Notify.make_monitor_notify(
                        '发送报警信息失败',
