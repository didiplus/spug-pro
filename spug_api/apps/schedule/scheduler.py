...
class Scheduler:
    ...
    def run(self):
        ...
        while True:
            try:
                _, data = rds_cli.brpop(settings.SCHEDULE_KEY, timeout=0)
            except Exception as e:
                logging.warning(f"Redis connection error in scheduler: {e}, reconnecting...")
                try:
                    rds_cli = get_redis_connection()
                    rds_cli.delete(settings.SCHEDULE_KEY)
                except Exception as reconnect_error:
                    logging.error(f"Scheduler reconnect failed: {reconnect_error}")
                    time.sleep(5)
                continue
