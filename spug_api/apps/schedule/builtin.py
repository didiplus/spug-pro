...
def auto_run_by_day():
    try:
        ...
        Alarm.objects.filter(created_at__lt=datetime.now() - timedelta(days=30)).delete()
        ...
        for item in Path(settings.TRANSFER_DIR).iterdir():
            if item.name != '.gitkeep':
                if item.stat().st_atime < timestamp:
                    transfer_dir = str(item.absolute())
                    # 使用 subprocess.run 替代 os.system，避免命令注入
                    try:
                        subprocess.run(['umount', '-f', transfer_dir], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False)
